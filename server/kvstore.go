...
func (s *KVStore) Get(ctx context.Context, key *pb.Key) (*pb.Result, error) {
	...
	// Create a request
	r := pb.Command{Cid: key.Cid, Operation: pb.Op_GET, Arg: &pb.Command_Get{Get: key}}
...
func (s *KVStore) Set(ctx context.Context, in *pb.KeyValue) (*pb.Result, error) {
	...
	c := make(chan pb.Result)
	// Create a request
	r := pb.Command{Cid: in.Cid, Operation: pb.Op_SET, Arg: &pb.Command_Set{Set: in}}
	// Send request over the channel
	s.C <- InputChannelType{command: r, response: c}
...
func (s *KVStore) Clear(ctx context.Context, in *pb.EmptyCid) (*pb.Result, error) {
	// Create a channel
	c := make(chan pb.Result)
	// Create a request
	r := pb.Command{Cid: in.Cid, Operation: pb.Op_CLEAR, Arg: &pb.Command_Clear{Clear: in}}
	// Send request over the channel
	s.C <- InputChannelType{command: r, response: c}
...
func (s *KVStore) CAS(ctx context.Context, in *pb.CASArg) (*pb.Result, error) {
	...
	c := make(chan pb.Result)
	// Create a request
	r := pb.Command{Cid: in.Kv.Cid, Operation: pb.Op_CAS, Arg: &pb.Command_Cas{Cas: in}}
	// Send request over the channel
	s.C <- InputChannelType{command: r, response: c}
...
func (s *KVStore) GetInternal(k string, cid int64) pb.Result {
	v := s.store[k]
	return pb.Result{Cid: cid, Result: &pb.Result_Kv{Kv: &pb.KeyValue{Key: k, Value: v}}}
}
...
func (s *KVStore) SetInternal(k string, v string, cid int64) pb.Result {
	s.store[k] = v
	return pb.Result{Cid: cid, Result: &pb.Result_Kv{Kv: &pb.KeyValue{Key: k, Value: v}}}
}

// Used internally, this function clears a kv store. Assumes no racing calls.
func (s *KVStore) ClearInternal(cid int64) pb.Result {
	s.store = make(map[string]string)
	return pb.Result{Cid: cid, Result: &pb.Result_S{S: &pb.Success{}}}
}

// Used internally this function performs CAS assuming no races.
func (s *KVStore) CasInternal(k string, v string, vn string, cid int64) pb.Result {
	vc := s.store[k]
	if vc == v {
		s.store[k] = vn
		return pb.Result{Cid: cid, Result: &pb.Result_Kv{Kv: &pb.KeyValue{Key: k, Value: vn}}}
	} else {
		return pb.Result{Cid: cid, Result: &pb.Result_Kv{Kv: &pb.KeyValue{Key: k, Value: vc}}}
	}
}
...
func (s *KVStore) HandleCommand(op InputChannelType) {
	...
	case pb.Op_GET:
		arg := c.GetGet()
		result := s.GetInternal(arg.Key, c.Cid)
		op.response <- result
	case pb.Op_SET:
		arg := c.GetSet()
		result := s.SetInternal(arg.Key, arg.Value, c.Cid)
		op.response <- result
	case pb.Op_CLEAR:
		result := s.ClearInternal(c.Cid)
		op.response <- result
	case pb.Op_CAS:
		arg := c.GetCas()
		result := s.CasInternal(arg.Kv.Key, arg.Kv.Value, arg.Value.Value, c.Cid)
		op.response <- result
	default:
